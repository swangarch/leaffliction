...
def balance_dataset(root_image_dir: str, aug_methods: list,
    ...
    plants = [d for d in os.listdir(root_image_dir)
              if os.path.isdir(os.path.join(root_image_dir, d))]
    if not plants:
        print("No plant directories found under", root_image_dir)
        return

    for plant in plants:
        plant_path = os.path.join(root_image_dir, plant)
        subtypes = [d for d in os.listdir(plant_path)
        ...
        counts = {}
        for st in subtypes:
            st_path = os.path.join(plant_path, st)
            files = list_files(st_path)
            counts[st] = len(files)
        ...
        print(f"Plant '{plant}': subtype counts = {counts}, target = {target}")

        for st in subtypes:
            st_path = os.path.join(plant_path, st)
            os.makedirs(st_path, exist_ok=True)
            current_files = list_files(st_path)
            current_count = len(current_files)
            need = target - current_count
            if need <= 0:
                continue

            print(f"  Augmenting '{st}' : need {need} images.")
            next_idx = next_image_index(st_path)
            method_cycle = [name for name, _ in aug_methods]

            base_pool = current_files.copy()
            if not base_pool:
                print(f"    WARNING: no base images in {st_path}, skipping.")
                continue

            for i in range(need):
                base_name = random.choice(base_pool)
                base_path = os.path.join(st_path, base_name)
                ...
                out_path = os.path.join(st_path, out_name)
