...
The project can run on Linux (WSL2) or MacOS,
If CUDA or MPS is avaliable, the it will choose automatically GPU, otherwise, choose CPU.
...
The project can be used for other dataset following this directory structure. 
It's tested on 38 categories full plantvillage dataset, 98% accuracy is achieved. 
...
-m to Choose model, -sp to split dataset with -a augmentation on training data. -t if there is test data.
If -sp not provided, training dataset and validation dataset need to be set as arguments.

```bash
python train.py leaves -m CNN -sp -a
# or
python train.py leaves -m RESNET -sp -a
```
...
The project provide a API service and a UI to serve the model, with uvicorn and FastAPI

Example:

```bash
python server.py --host 0.0.0.0 --port 8011 --model RESNET -l weights.pth
```

To serve the pretrained model on full plantvillage dataset with 38 categories:
```bash
python server.py -l weights/weights_resnet_38cat.pth -m RESNET -lc categories/categories_38.json   
```
...
```bash
curl -X POST http://localhost:8011/leaffliction/predict   -F "file=@grape_esca.JPG"
```

```
{"result":{"catID":2,"category":"Apple___Cedar_apple_rust","success":true}}
```
To access the UI from a browser
```bash
http://localhost:8011/leaffliction
```

![Page](./visualize/Index.png)
![Page](./visualize/Result.png)
