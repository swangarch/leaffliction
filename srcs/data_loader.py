...
def create_dataloader(path_train: str, path_val: str,
                      batch_size: int = 64) -> tuple[tuple[DataLoader,
                                                           DataLoader], int]:
    ...
    count."""
    dataset_train = create_dataset(path_train)
    dataset_val = create_dataset(path_val)
    train_loader = DataLoader(dataset_train, batch_size=batch_size,
                              shuffle=True)
    val_loader = DataLoader(dataset_val, batch_size=batch_size,
                            shuffle=True)
    save_categories(dataset_train.classes)
    return (train_loader, val_loader), len(dataset_train.classes)


def batch_test_dataloader(path: str, batch_size:
                          int = 64) -> Tuple[DataLoader, datasets.ImageFolder]:
...
def load_categories(path: str = "./tmp/categories.json") -> List[str]:
    ...
    if not os.path.exists(path):
        raise RuntimeError("No categories file, cannot determine catergories.")
